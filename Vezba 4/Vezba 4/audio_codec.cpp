...
#define BLOCK_SIZE (1<<(MDCT_ORDER-1))

short wav_buffer[BLOCK_SIZE];
double window[MDCT_SIZE];
...
void main()
{
  ...
  for (i=0; i<(1<<(MDCT_ORDER-2)); i++)
    bitreverse_split_radix[-split_radix_permutation(i, 1<<(MDCT_ORDER-2), 0) & ((1<<(MDCT_ORDER-2))-1)] = i;
